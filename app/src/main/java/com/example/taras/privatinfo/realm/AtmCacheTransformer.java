...
public class AtmCacheTransformer implements Observable.Transformer<List<DeviceDB>, List<DeviceDB>> {
    private final String LOG_TAG = "myLogs";

    private final Func1<List<DeviceDB>, Observable<List<DeviceDB>>> mSaveFunc = devices -> {
        Realm.getDefaultInstance().executeTransaction(realm -> {
            /*RealmResults<DeviceDB> realmResults = realm.where(DeviceDB.class).equalTo("type", "ATM").findAll();
            realmResults.deleteAllFromRealm();
            realm.insert(devices);*/
            realm.delete(DeviceDB.class);
            realm.insert(devices);
    ...
    private final Func1<Throwable, Observable<List<DeviceDB>>> mCacheErrorHandler = throwable -> {
        ...
        //RealmResults<DeviceDB> results = realm.where(DeviceDB.class).equalTo("type", "ATM").findAll();
